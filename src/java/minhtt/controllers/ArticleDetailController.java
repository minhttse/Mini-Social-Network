...
public class ArticleDetailController extends HttpServlet {
    private static final String SUCCESS = "article.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ArticleDTO dto = dao.performArticleDetails(articleID);
            List<CommentDTO> list = cmtDao.loadAllCommentByArticleID(articleID);
            session.setAttribute("COMMENT", list);
            if (dto != null){
                session.setAttribute("ARTICLE", dto);
                url = SUCCESS;
            }
        } catch (Exception e) {
            log("Error at ArticleDetailServlet: " + e.toString());
        } finally {
